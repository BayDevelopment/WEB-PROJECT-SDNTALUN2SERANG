...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        const tableEl = document.getElementById('tableDataKelas');
        if (!tableEl) return;
        ...
        const dt = new DataTable(tableEl, {
            ...
            buttons: [{
                ...
                filename: () => {
                    ...
                    return `Laporan_Kelas_${d}`;
                },
                exportOptions: {
                    // 0=No, 1=Nama Kelas, 2=Tingkat, 3=Jurusan, 4=Aksi
                    columns: [0, 1, 2, 3], // exclude kolom Aksi
                    modifier: {
                        search: 'applied',
                        order: 'applied',
                        page: 'all'
            ...
            order: [
                [1, 'asc'],
                [2, 'asc']
            ], // urut default: Nama Kelas lalu Tingkat
        ...
        // Sesuaikan lebar/responsive setelah init
        dt.on('init', () => {
            dt.columns.adjust();
            dt.responsive.recalc();
        });

        // Debounce pencarian bawaan DataTables agar halus
        const searchInput = document.querySelector('#tableDataKelas_filter input[type="search"]');
        if (searchInput) {
